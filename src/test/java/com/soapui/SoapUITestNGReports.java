...
public class SoapUITestNGReports {
	...
	try {
	    project = new WsdlProject("src/main/resources/soapui/demo-project.xml");
    ...
    public void testSoapProject_ByTest(String suiteName, String testName) throws XmlException, IOException, SoapUIException{	
	
	Sleeper.sleep(250);

	TestRunner runner  = project.getTestSuiteByName(suiteName).getTestCaseByName(testName).run(new PropertiesMap(), false);
	TestReporter.assertTrue(runner.getStatus().toString().equals("FINISHED"), "Test was [ " + runner.getStatus() + " ] and expected [ FINISHED ]. Error reason [ " + runner.getReason() + " ]");
